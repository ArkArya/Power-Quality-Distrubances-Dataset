% function xs = c20_sag_with_harmonics_with_flicker( )
            ...
            for alpha5=0.05:0.05:0.15
                for lamda=0.05:0.01:0.1
                    for fn=8:4:25
                        ...
                        x=@(t) sin(2*pi*50*t) + (1 + lamda* sin(2*pi*fn*t)) .* (alpha1*sin(1*2*pi*50*t-vn) + alpha3*sin(3*2*pi*50*t-vn) + alpha5*sin(5*2*pi*50*t-vn)) .* ( (-1)* alpha *(heaviside(t-t1)-heaviside(t-t2))); %% Generated Signal
                        flag = flag + 1;

%                                                 % % visualisation
%                                                 if flag > 2026 && flag < 2050
%                                                     figure(flag)
%                                                     plot(t,x(t));
%                                                     hold on;
%                                                 end
